...
// Perform queries 
$sonuc=mysqli_query($con,"SELECT * FROM users_tv,tvshow where users_tv.tv_id=tvshow.id AND users_id='$id'");
while($movie = mysqli_fetch_array($sonuc))
{
                   
      ?>

<a href="liketv.php?idl=<?=$tv['id']?>">LIKE </a>
<?php                       

echo("<img src='" . $config['images']['base_url'] . $config['images']['poster_sizes'][2] . $movie['tvposterpath'] ."'/>");
echo  $movie["tvname"]."<br>"."<br />"."<br />";
