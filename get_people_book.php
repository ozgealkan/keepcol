...
// Perform queries 
$sonuc=mysqli_query($con,"SELECT * FROM book_user,books WHERE book_user.book_id=books.id AND user_id='1'");
while($book = mysqli_fetch_array($sonuc))
{
              
     ?>
<a href="liketv.php?idl=<?=$movie['id']?>">LIKE </a>
    <?php                          


echo("<img src='" . $config['images']['base_url'] . $config['images']['poster_sizes'][2] . $book['img'] ."'/>");
