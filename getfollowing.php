...
<?php
    ...
    $sonuc = mysqli_query($con, "SELECT * FROM following, users WHERE following.user_id = users.id AND following.userid = '$userId'");

    while($movie = mysqli_fetch_array($sonuc)){
        echo  $movie["first_name"]."<br>"."<br />"."<br />";
?>

<a href="people_mycollection.php?idm=<?=$movie['id']?>"><?php echo  $movie["first_name"]; echo  $movie["last_name"]."<br />"."<br />"."<br />";?></a>
<?php                       
    //echo("<img src='" . $config['images']['base_url'] . $config['images']['poster_sizes'][2] . $movie['tvposterpath'] ."'/>");
